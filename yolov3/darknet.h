...
torch::nn::Conv2dOptions conv_options(int64_t in_planes, int64_t out_planes, int64_t kerner_size, int64_t stride, int64_t padding, int64_t groups, bool with_bias=false){
...
struct darknet_yolo : torch::nn::Module {
 public:
   ...
   darknet_yolo(const char *cfg_file, torch::Device *device) {
      load_cfg(cfg_file);
      _device = device;
      create_modules();
      }
   ...
   void load_weights(const char *weight_file) {
      ...
      at::TensorOptions options= torch::TensorOptions()
         .dtype(torch::kFloat32);//.is_variable(true);
